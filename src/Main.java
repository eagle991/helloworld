...
public class Main {
    ...
    private static String getRecommendApplicationVersion(int deviceYear, int clientOs) {
        if (clientOs < 0 || clientOs > 1) return null;
        int currentYear = LocalDate.now().getYear();
        if (deviceYear < 1990 && deviceYear > currentYear + 1) return null;
